...
class NameValidatorTest extends TestCase {
    public function testValidName() {
        $validator = new NameValidator('John');
        $this->assertEquals([true, "Valid Name - Back End"], $validator->validateName());
    }

    public function testInvalidName() {
        $validator = new NameValidator('J');
        $this->assertEquals([false, "Name must be between 2 and 50 characters - Back End"], $validator->validateName());
    }

    public function testInvalidCharactersInName() {
        $validator = new NameValidator('John123');
        $this->assertEquals([false, "Name can only contain letters - Back End"], $validator->validateName());
    }
